...
def Fig4(x, i_c, mu_train, mu_test, X_train, X_train_s, X_test, X_test_ROM, X_test_sROM):
    ...
    indices_l, norm_distances_l = find_indices((mu_train.ravel(),), mu_test[i_c, None])
    i_l = indices_l.ravel()  # index snapshot left
    for _X, _X_R, lbl, col in zip(
        [X_train, X_train_s],
        [X_test_ROM, X_test_sROM],
        [r"$u_{\mu,rb}$: standard ROM", r"$u_{\mu, rb, S}$: regularized ROM"],
        ["C0-", "C1--"],
    ):
        ...
        plt.plot(
            x, X_test[:, i_c], "k-", ms=4, markevery=markevery, label="$u_{\mu}$: true solution"
        )
        ...
        plt.plot(
            ...
            lw=0.5,
            ms=4,
            markevery=(30, markevery),
            label="$u_{2}$: snapshot \#3",
        )
        plt.plot(
            x,
            _X[:, i_l + 1],
        ...
        plt.legend()
        e = np.mean((X_test[:, 0] - _X_R[:, 0]) ** 2) ** 0.5
        ax.text(
            0.35,
            0.35,
            "$\|u_{\mu}-$" + lbl.split(":")[0] + "$\|_{L_2}" + "={:.2f}$".format(e),
            fontsize=8,
            va="center",
        )
        plt.xlabel("x")
...
def Fig5(x, X_test, X_test_s, X_test_ROM, X_test_sROM, X_test_sROMs):
    for j in [5]:  # [0, 2, 5, 7]:
        ...
        fig, ax = plt.subplots(figsize=(page_width_in / 2, page_width_in / 3))
        plt.plot(
            x, X_test[:, j], "k-", ms=4, markevery=(0, markevery), label="$u_{\mu}$"
        )  # : true solution
        # : regularized true solution
        # plt.plot(x, X_test_s[:, j], "k--", ms=4,
        #          markevery=(20, markevery), label="$u_{\mu, S}$")
        plt.plot(
            x, X_test_ROM[:, j], "C0-", ms=4, markevery=(10, markevery), label="$u_{\mu, rb}$"
        )  # : standard ROM
        # : regularized ROM
        ...
        e = np.mean((X_test[:, j] - X_test_ROM[:, j]) ** 2) ** 0.5
        ax.text(
            0.35,
            0.4,
            "$\|u_{\mu}{-u_{\mu, rb}}\|_{L_2}" + "={:.3f}$".format(e),
            fontsize=8,
            va="center",
        )
        ...
        e = np.mean((X_test[:, j] - X_test_sROMs[:, j]) ** 2) ** 0.5
        ax.text(
            0.35,
            0.25,
            "$\|u_{\mu}{-u_{\mu, rb, DS}}\|_{L_2}" + "={:.3f}$".format(e),
            fontsize=8,
            va="center",
        )
...
def Fig6(mu_val, eROM, esROM, esROMs, esROMs2):
    ...
    ax.plot(mu_val, esROMs, "C2-", label="$u_{\mu, rb, SD_{100}}$")
...
def Fig7(eROM, esROM, esROMs, c_all, sigmas):
    ...
    print(np.min(Z), "%")
    cs = ax.imshow(Z, interpolation="nearest", origin="lower", vmin=np.min(Z), vmax=0, extent=ext)
    lvls = np.linspace(np.min(Z) // 5 * 5, 0, 4)
    contours = ax.contour(Z, levels=lvls, colors=["g", "g", "g", "r"], extent=ext)
...
def Fig8(NN, dN_ROM, dN_sROMs, dN_sROMs2):
    fig, ax = plt.subplots(figsize=(page_width_in / 2, page_width_in / 3))
    plt.plot(NN, dN_ROM, "C0.-", ms=2, label="$u_{\mu,rb}$")
    # plt.plot(NN, esROM, "C1--", ms=2, label="$u_{rb,S}$")
    plt.plot(NN, dN_sROMs, "C2.-", ms=2, label="$u_{\mu,Srb,D_{100}}$")
