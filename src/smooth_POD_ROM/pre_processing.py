...
def on_regular_grid(points, triangles, data, **kvargs):
    cell_centers = get_centers(points, triangles)
    ...
    data_on_grid = griddata(cell_centers[:, :2], data,
                            (X.ravel(), Y.ravel()), **kvargs)
